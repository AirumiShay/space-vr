...
#Принцесса создаёт сцену - гнездо Роя
func _on_NestTimer_timeout():
	if get_tree().is_network_server():	
				Network.create_bullet_player0(self.faction,global_position-Vector2(100,100),$Area2D.rotation,6)	 # 6 - base nest swarm	
				Network._send_Creature_Dead(self.name,Global.type_update_robot_swarm)
				dead_now = true
				$DieTimer.start(1)
