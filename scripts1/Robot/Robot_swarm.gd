...
#Это сцена вирус - робот-воин Роя
func _ready():
#	$DamageTimer.start(5)
...
func patrol(robot,delta):
	...
	if target_base != null:
		...
		robot.global_position += direction * robot.speed * delta
		Network._send_position_robot_swarm(global_position,uuid)		
...
remote func set_new_position_robot_swarm(pos):
		if not get_tree().is_network_server():
#			move_and_slide(direction * 1000)
			global_position = pos #+= direction * speed * delta
#			rpc_unreliable("position", position)			
...
func _process(delta):
	if not	get_tree().is_network_server() or 	dead_now == true:
		return	
	if timer < 0.10:
		timer += delta
	else:
...
func a_on_DamageTimer_timeout():
	if get_tree().is_network_server():
		var attack = $DamageNPC
		attack._attack_true()
		$DamageNPC/DamageSprite.visible = false
